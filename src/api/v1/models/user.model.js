...
const isUserExist = (username, email) => {
    ...
    users = users.filter(user => user.username == username || user.email == email)
...
const isPasswordCorrect = async (password) => {
    const hashedPassword = await bcrypt.hash(password, 10)    // Remove this hard coded value. Once testing done
